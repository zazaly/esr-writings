::: navheader
  ...
  ---------------------------------------------- ---------------------- --------------------------------------
  [Prev](contrastchapter.html){accesskey="p"}     Chapter 3. Contrasts     [Next](ch03s02.html){accesskey="n"}
...
## []{#id2891992}The Elements of Operating-System Style {#the-elements-of-operating-system-style .title style="clear: both"}
...
### []{#id2892028}What Is the Operating System\'s Unifying Idea? {#what-is-the-operating-systems-unifying-idea .title}

</div>
::::

Unix has a couple of unifying ideas or metaphors that shape its APIs and the development style that proceeds from them. The most important of these are probably the "everything is a file" model and the pipe metaphor[]{#id2892043 .indexterm}^\[[20](ch03s01.html#ftn.id2892049){#id2892049}\]^ built on top of it. In general, development style under any given operating system is strongly conditioned by the unifying ideas baked into the system by its designers --- they percolate upwards into applications programming from the models provided by system tools and APIs.
...
### []{#id2892085}Multitasking Capability {#multitasking-capability .title}
...
### []{#id2892171}Cooperating Processes {#cooperating-processes .title}

</div>
::::

In the Unix experience, inexpensive process-spawning and easy inter-process communication (IPC) makes a whole ecology of small tools, pipes[]{#id2892182 .indexterm}, and filters possible. We\'ll explore this ecology in [Chapter 7](multiprogramchapter.html "Chapter 7. Multiprogramming"); here, we need to point out some consequences of expensive process-spawning and IPC.
...
::: blockquote
  ...
  \--[ [Doug McIlroy]{.author} []{#id2892213 .indexterm} ]{.attribution}                                                                                                                                                                                                                                                                                                                                                           
  ------------------------------------------------------------------------ ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ ---
:::
...
-   Lots of policy has to be expressed within those monoliths. This encourages C++[]{#id2888005 .indexterm} and elaborately layered internal code organization, rather than C and relatively flat internal hierarchies.
...
::: blockquote
  ...
  \--[ [Doug McIlroy]{.author} []{#id2888096 .indexterm} ]{.attribution}                                                                                                                                                                                                                                                                                 
  ------------------------------------------------------------------------ ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ---
:::

We\'ll return to this theme in [Chapter 7](multiprogramchapter.html "Chapter 7. Multiprogramming").
...
### []{#id2888136}Internal Boundaries {#internal-boundaries .title}

</div>
::::

Unix has wired into it an assumption that the programmer knows best. It doesn\'t stop you or request confirmation when you do dangerous things with your own data, like issuing **rm -rf \***. On the other hand, Unix is rather careful about not letting you step on other people\'s data. In fact, Unix encourages you to have multiple accounts, each with its own attached and possibly differing privileges, to help you protect yourself from misbehaving programs.^\[[22](ch03s01.html#ftn.id2888159){#id2888159}\]^ System programs often have their own pseudo-user accounts to confer access to special system files without requiring unlimited (or *superuser*) access.
...
### []{#id2888217}File Attributes and Record Structures {#file-attributes-and-record-structures .title}
...
OS-level record structures are generally an optimization hack[]{#id2888248 .indexterm}, and do little more than complicate APIs and programmers\' lives. They encourage the use of opaque record-oriented file formats that generic tools like text editors cannot read properly.

File attributes can be useful, but (as we will see in [Chapter 20](http://www.catb.org/~esr/writings/taoup/html/futurechapter.html "Chapter 20. Futures")) can raise some awkward semantic issues in a world of byte-stream-oriented tools and pipes[]{#id2888272 .indexterm}. When file attributes are supported at the operating-system level, they predispose programmers to use opaque formats and lean on the file attributes to tie them to the specific applications that interpret them.
...
### []{#id2888298}Binary File Formats {#binary-file-formats .title}

</div>
::::

If your operating system uses binary formats for critical data (such as user-account records) it is likely that no tradition of readable textual formats for applications will develop. We explain in more detail why this is a problem in [Chapter 5](textualitychapter.html "Chapter 5. Textuality"). For now it\'s sufficient to note the following consequences:

::: itemizedlist
-   Even if a command-line interface, scripting, and pipes[]{#id2888330 .indexterm} are supported, very few filters will evolve.
...
::: itemizedlist
-   Programs will not be designed to cooperate with each other in unexpected ways --- because they [*can\'t*]{.emphasis} be. Outputs aren\'t usable as inputs.

-   Remote system administration will be sparsely supported, more difficult to use, and more network-intensive.^\[[23](ch03s01.html#ftn.id2888440){#id2888440}\]^
...
### []{#id2888484}Intended Audience {#intended-audience .title}
...
::: blockquote
  ...
  \--[ [Ken Thompson]{.author} []{#id2888546 .indexterm} ]{.attribution}                                                                                                                                                                   
  ------------------------------------------------------------------------ -------------------------------------------------------------------------------------------------------------------------------------------------------------- ---
:::
...
### []{#id2888581}Entry Barriers to Development {#entry-barriers-to-development .title}
...
::: footnote
^\[[20](ch03s01.html#id2892049){#ftn.id2892049}\]^ For readers without Unix experience, a pipe is a way of connecting the output of one program to the input of another. We\'ll explore the ways this idea can be used to help programs cooperate in [Chapter 7](multiprogramchapter.html "Chapter 7. Multiprogramming").
:::

::: footnote
^\[[21](ch03s01.html#id2892115){#ftn.id2892115}\]^ A periodic clock interrupt from the hardware is useful as a sort of heartbeat for a timesharing system; each time it fires, it tells the system that it may be time to switch to another task, defining the size of the unit timeslice. In 2003 Unixes usually set the heartbeat to either 60 or 100 times a second.
:::

::: footnote
^\[[22](ch03s01.html#id2888159){#ftn.id2888159}\]^ The modern buzzword for this is *role-based security*.
:::

::: footnote
^\[[23](ch03s01.html#id2888440){#ftn.id2888440}\]^ This problem was considered quite serious by Microsoft itself during their rebuild of Hotmail. See \[[BrooksD](http://www.catb.org/~esr/writings/taoup/html/apb.html#BrooksD "[BrooksD]")\].
:::
:::::::
:::::::::::::::::::::::::::::::::::::::::::
...
------------------------------------------------------------------------
  ...
  ---------------------------------------------- ------------------------------------------- --------------------------------------
  [Prev](contrastchapter.html){accesskey="p"}     [Up](contrastchapter.html){accesskey="u"}     [Next](ch03s02.html){accesskey="n"}
  Chapter 3. Contrasts                                [Home](index.html){accesskey="h"}                Operating-System Comparisons
  ---------------------------------------------- ------------------------------------------- --------------------------------------
:::
