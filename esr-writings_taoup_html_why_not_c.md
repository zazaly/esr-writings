::: navheader
  Why Not C?                                                     
  -------------------------------------- ----------------------- --------------------------------------
  [Prev](ch14s01.html){accesskey="p"}     Chapter 14. Languages     [Next](ch14s03.html){accesskey="n"}
...
## []{#why_not_c}Why Not C? {#why-not-c .title style="clear: both"}

</div>
::::

[]{#id2970988 .indexterm}[]{#id2970999 .indexterm}
...
C memory management is an enormous source of complication and error. One study (cited in \[[Boehm](http://www.catb.org/~esr/writings/taoup/html/apb.html#Boehm "[Boehm]")\]) estimates that 30% or 40% of development time is devoted to storage management for programs that manipulate complex data structures. This did not even include the impact on debugging cost. While hard figures are lacking, many experienced programmers believe that memory-management bugs are the single largest source of persistent errors in real-world code.^\[[124](why_not_c.html#ftn.id2968237){#id2968237}\]^ Buffer overruns are a common cause of crashes and security holes. Dynamic-memory management is particularly notorious for spawning insidious and hard-to-track bugs, such as memory leaks and stale-pointer problems.
...
A recent paper \[[Prechelt](http://www.catb.org/~esr/writings/taoup/html/apb.html#Prechelt "[Prechelt]")\] musters an impressive array of statistical evidence for a claim that programmers with experience in both worlds will find very plausible: programmers are just about twice as productive in scripting languages[]{#id2968281 .indexterm} as they are in C or C++. This accords well with the 30%--40% penalty estimate noted earlier, plus debugging overhead. The performance penalty of using a scripting language[]{#id2968293 .indexterm} is very often insignificant for real-world programs, because real-world programs tend to be limited by waits for I/O events, network latency, and cache-line fills rather than by the efficiency with which they use the CPU itself.

The Unix world has been slowly coming around to this point of view in practice, especially since 1990 or so, as is shown by the increasing popularity of Perl and other scripting languages. But the evolution of practice has not yet (as of mid-2003) led to a wholesale change in conscious attitudes; many Unix programmers are still absorbing the lesson Perl[]{#id2968316 .indexterm} and Python[]{#id2968325 .indexterm} have been teaching.

We can see the same trend happening, albeit more slowly, outside the Unix world --- for example, in the continuing shift from C++ to Visual Basic evident in applications development under Microsoft Windows and NT[]{#id2968341 .indexterm}, and the move toward Java[]{#id2968350 .indexterm} in the mainframe world.
...
::: footnote
^\[[123](why_not_c.html#id2971041){#ftn.id2971041}\]^ Outside the Unix world, this three-orders-of-magnitude improvement in hardware performance has been masked to a significant extent by a corresponding drop in software performance.
:::

::: footnote
^\[[124](why_not_c.html#id2968237){#ftn.id2968237}\]^ The severity of this problem is attested to by the rich slang Unix programmers have developed for describing different varieties: 'aliasing bug', 'arena corruption', 'memory leak', 'buffer overflow', 'stack smash', 'fandango on core', 'stale pointer', 'heap trashing', and the rightly dreaded 'secondary damage'. See the [Jargon File](http://www.catb.org/~esr/jargon){target="_top"} for elucidation.
:::
:::::
::::::::
...
------------------------------------------------------------------------
  ...
  -------------------------------------- -------------------------------------------- ---------------------------------------------
  [Prev](ch14s01.html){accesskey="p"}     [Up](languageschapter.html){accesskey="u"}            [Next](ch14s03.html){accesskey="n"}
  Unix\'s Cornucopia of Languages             [Home](index.html){accesskey="h"}          Interpreted Languages and Mixed Strategies
  -------------------------------------- -------------------------------------------- ---------------------------------------------
:::
