::: navheader
  ...
  -------------------------------------- -------------------------- ------------------------------------------------
  [Prev](ch12s03.html){accesskey="p"}     Chapter 12. Optimization     [Next](complexitychapter.html){accesskey="n"}
...
## []{#id2961183}Throughput vs. Latency {#throughput-vs.-latency .title style="clear: both"}
...
### []{#id2961306}Batching Operations {#batching-operations .title}
...
### []{#binary_caches}Caching Operation Results {#caching-operation-results .title}
...
While this approach can be made to work, it has all the disadvantages that the SPOT[]{#id2961469 .indexterm} rule would lead us to expect. The duplication of data means that it doesn\'t yield any economy of storage --- it\'s purely a speed optimization[]{#id2961480 .indexterm}. But the real problem with it is that the code to ensure coherency between cache and master is notoriously leaky and bug-prone. Very frequently updated cache files can lead to subtle race conditions simply because of the 1-second resolution of timestamps.

Coherency can be guaranteed in simple cases. One such is the Python interpreter[]{#id2961499 .indexterm}, which compiles and deposits on disk a p-code file with extension `.pyc` when a Python library file is first imported. On subsequent runs the cached copy of the p-code is loaded unless the source has since changed (this avoids reparsing the library source code on every run). Emacs Lisp uses a similar technique with `.el` and `.elc` files. This technique works because both read and write accesses to the cache go through a single program.
...
------------------------------------------------------------------------
  ...
  -------------------------------------- ----------------------------------------------- ------------------------------------------------
  [Prev](ch12s03.html){accesskey="p"}     [Up](optimizationchapter.html){accesskey="u"}     [Next](complexitychapter.html){accesskey="n"}
  Nonlocality Considered Harmful                [Home](index.html){accesskey="h"}                                  Chapter 13. Complexity
  -------------------------------------- ----------------------------------------------- ------------------------------------------------
:::
